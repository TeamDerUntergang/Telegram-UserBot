...
async def unmute_chat(unm_e):
    ...
    try:
        from sedenbot.moduller.sql_helper.keep_read_sql import unkread
...
async def mute_chat(mute_e):
    ...
    try:
        from sedenbot.moduller.sql_helper.keep_read_sql import kread
    except AttributeError:
...
async def keep_read(message):
    """ Mute mantığı. """
    try:
        from sedenbot.moduller.sql_helper.keep_read_sql import is_kread
    except AttributeError:
        return
    kread = is_kread()
    if kread:
